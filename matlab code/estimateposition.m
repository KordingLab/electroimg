...
for i=1:numiter
    ...
    %x0 = reshape([randn(k,3)*10^-8, randn(k,1)],k*4,1);
    fx = @(x)myfun3(x,y,grid_coord,k,lambda);
    opts.dim=k*4; numiter = 1000;
    [finalx finaly t] = oo(fx,numiter,opts);
...
function fx = myfun3(x,y,grid_coord,k,lambda)
...
for j=1:k
for i=1:n
    fx = fx + (y(i) - x2(j,4)./norm(grid_coord(i,:)-x2(j,1:3))).^2; 
end
end

% enforce constraint that transmembrane currents must sum to 0
fx = fx + lambda*sum(x2(:,4)); 
